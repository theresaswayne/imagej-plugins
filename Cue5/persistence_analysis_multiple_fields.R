# R script to calculate persistence of cell interactions
# Assumptions:
#   Input is a  CSV file generated by combine_csv_files_with_num.R
#   Required cols: filename, Label, timepoint (others are ignored)
#   filename data contains Well and ROI name in the pattern Wellxx ROIy FrameZZZ.csv
#   Label data is of the format objA1_objB7 
# Outputs:
#  Table will be saved in the parent directory of the input folder.
#  Data calculated: 
#    number and % of objects in the A class that show interactions
#    number and % of objects in the A class that show persistent interactions 

# Parameters:         
#   Edit object names, time window, interaction threshold in the Parameters section
...
# Use filename to identify Well and ROI

patt <- "^Well(?<well>[A-H][0-9]{1,2}) ROI(?<roi>[1-9]{1})"
 
wells <- str_match(df$filename,patt)[,2]
rois <- str_match(df$filename,patt)[,3]

# insert the new data
df <- df %>% mutate(Well = wells,
                    ROI = rois)

# ---- Count interactions ----

# total interactions per timepoint
intPerTime <- df %>% group_by(Well, ROI) %>%
  count(timepoint)

# total interactions per T cell (objA)
intPerA <- df %>% group_by(Well, ROI) %>%
  count(ObjA) %>% 
...
for (obj in objA_IDs) {
  ...
  objInts <- df %>%
    filter(ObjA == obj) # select rows pertaining to one objA in one movie
