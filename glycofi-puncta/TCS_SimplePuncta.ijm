...
//a simpler version that counts gross total inclusions and cells in a field
...
saveAs("Tiff", dirName+imageName+" cells.tif"); // this is also an 8-bit image
...
inclusionThresh=CellMean+2*CellStd;

print("Name \t ImgMean \t ImgSD \t CellMean \t CellSD \t CellThresh \t InclThresh");
print(imageName+"\t"+ImgMean+"\t"+ImgStd+"\t"+CellMean+"\t"+CellStd+"\t"+lower+"\t"+inclusionThresh);

// replaced with an artificial threshold while using test image
// may also want to use local thresholding on this part
// inclusionThresh=63000;

// STEP 3. Threshold and measure inclusions.

setThreshold(inclusionThresh,ImgMax);		// Threshold for inclusionsrun("Analyze Particles...", "size=0-1000 circularity=0.00-1.00 show=Masks exclude summarize");
selectWindow("Mask of "+imageName+".tif");
saveAs("Tiff", dirName+imageName+" incl.tif");
run("Create Selection");
roiManager("Add"); // the inclusions are the 2nd ROI in the list, number 1
close();

selectWindow("Summary");
saveAs("Text", dirName+imageName+" Summary .xls"); 
...
run("Flatten");
saveAs("Tiff", dirName+imageName+" overlay.tif");
...
close();
selectWindow("Log");
saveAs("Text", dirName+imageName+" Log.xls"); 
// print("\\Clear");
// run("Close");
