...
# @String(label="Process folders containing",description="Clear for no filtering",value=".tif") folderFilter
...
def getFileList(directory, filteringString):
    print("Looking for " + filteringString)
    files = []
    for (root, dirs, filenames) in os.walk(directory):
        if out_dir in dirs: # Ignore destination directory
            dirs.remove(out_dir)
        for d in dirs:
			if filteringString in d:
				print("the winning directory is " + d)
				for f in os.listfiles(d):
				    print(f)
				    if f.is_file():
				        files.append(f.path)
    return files
...
files = getFileList(src_dir, folderFilter)
...
if files:
    ...
    csvPath = os.path.join(out_dir, "_ProcessedFileList.csv")
    ...
    for (counter, f) in enumerate(files):
        ...
        newpath = os.path.join(out_dir, os.path.splitext(imp.getTitle())[0] + ".tif")
    ...
    IJ.error("No matches for the selected text.")
