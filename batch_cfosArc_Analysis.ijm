   ...
   for (i=0; i<list.length; i++) {
        if (endsWith(list[i], "/"))
        	...
        	if (startsWith(list[i], "C1"))
           		processArcImage(dir1, list[i]);
           	else if (startsWith(list[i], "C2")) {
           		processCfosImage(dir1, list[i]);
           	}
    }
}

function processCfosImage(dir1, name) {
   open(dir1+name);
   print(n++, name);
...
function processArcImage(dir1, name) {
   open(dir1+name);
   print(n++, name);
