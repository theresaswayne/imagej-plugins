...
# shows how to make multi-point ROIs in the ROI manager
...
# roi = PointRoi(0, 0)  
# problem --  this generates a bad point
# It's possible to avoid this by making each discovered point into a separate roi like this:
# 	proi = PointRoi(peak.getDoublePosition(0) / cal.pixelWidth, peak.getDoublePosition(1) / cal.pixelHeight)
# 	rm.addRoi(proi)



# but first lets try a simple tip from https://syn.mrc-lmb.cam.ac.uk/acardona/fiji-tutorial/
roi = PointRoi()

# But if you want to create multi-point ROIs we may need to collect all coordinates at once
#multipointX = zeros(len(peaks), 'i')
#multipointY = zeros(len(peaks), 'i')
