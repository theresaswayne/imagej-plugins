...
/*
 * - opens multi-point ND2 files and saves individual TIFs for each point and channel, assuming 2 channels
...
for (i=0; i<list.length; i++) // loop through all files in the folder
    ...
    for (j=1; j<=seriesCount; j++) // open each series 
		...
		rename(seriesname);
		run("Split Channels");
		selectWindow("C1-"+seriesname);
        saveAs("Tiff", outputDir + File.separator + name + "_C1_XY" + seriesname + ".tif");    
		selectWindow("C2-"+seriesname);
        saveAs("Tiff", outputDir + File.separator + name + "_C2_XY" + seriesname + ".tif"); 
        run("Close All");
        run("Collect Garbage");
    	}
	}
